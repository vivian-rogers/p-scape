#!/usr/bin/env bash
# Vienna pilot on the K=48 jittered MC pipeline. First-time city — downloads
# the Austria OSM extract on the first build_tiles.sh invocation. All 7 grids
# end-to-end ≈ 28-32 min.
#
# Run from pnorm/:
#   caffeinate -d -i -s -u bash ./scripts/rerun_vienna_sq.sh
...
LOG="data/rerun_vienna_sq_${TS}.log"
mkdir -p data
exec > >(tee -a "$LOG") 2>&1
ts() { date +%H:%M:%S; }
...
echo "[$(ts)] === rerun_vienna_sq starting; log → $LOG ==="
